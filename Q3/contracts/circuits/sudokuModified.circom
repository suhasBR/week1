...
include "/home/suhas/zku/week1 assignment/week1/Q3/contracts/circuits/RangeProof.circom";
...
template sudoku() {
    ...
    component rPuzzle[9][9];
    component rSoln[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rPuzzle[i][j] = RangeProof(4);
            rSoln[i][j] = RangeProof(4);

            //number to be checked to in
            rPuzzle[i][j].in <== puzzle[i][j];

            //range[0] is lower bound, range[1] is upper bound
            rPuzzle[i][j].range[0] <== 0;
            rPuzzle[i][j].range[1] <== 9;

            //check if output is 1
            rPuzzle[i][j].out === 1;

            rSoln[i][j].in <== solution[i][j];
